...
impl TGSAnimation {
    pub fn load_from_tgs_bytes(bytes: Vec<u8>) -> Result<TGSAnimation, Box<dyn Error>> {
        Ok(bytes)
            .and_then(|bytes| TGSAnimation::decompress_if_gzip(bytes).map_err(|_| GzipDecompressionError {}.into()))
            .and_then(|content| Animation::from_data(content, "", "").ok_or(LottieParseError {}.into()))
            .and_then(|animation| Ok(TGSAnimation { lottie: animation }))
    }

    pub fn export_to_gif_bytes(self) -> Result<Vec<u8>, Box<dyn Error>> {
        let mut buffer =
            Vec::with_capacity(self.lottie.size().height * self.lottie.size().width * self.lottie.totalframe());
